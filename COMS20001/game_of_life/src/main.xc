...
/////////////////////////////////////////////////////////////////////////////////////////
//
// Program that evolves the game of life for an infinite number of iterations
//
/////////////////////////////////////////////////////////////////////////////////////////
...
#define nb_workers 8               //number of worker threads
#define strip_size IMHT/nb_workers //how we split the image (size of one strip belonging to one worker)
#define size_char (sizeof(uchar)*8)//size of one uchar in bits
#define nb_rounds 2                //number of rounds desired (no longer used, was used for debugging/getting proper output)
...
#define INFRM "64x64.pgm"           //name of input image
#define OUTFRM "64x64outout.pgm"    //name of output image
...
/////////////////////////////////////////////////////////////////////////////////////////
//
// Distributor receives and sends out image (from DataInputStream to workers, from workers to DatOutputStream),
// notifies workers if user wants image to be output
//
/////////////////////////////////////////////////////////////////////////////////////////
void distributor(chanend c_in, chanend c_out, chanend c_workers[nb_workers], chanend fromAcc, chanend c_button, chanend c_leds)
{
  uchar val = 0;//variable to receive and send data, also used as a flag to detect the button press
  int serving = 1;
  ...
  uchar work = 0;//value used to keep workers working
  uchar sendImage = 1;//value used to tell workers to send over their part of the image
  ...
  while(serving){
      ...
      select {
          ...
          //tentative of making a separate timer thread, could not finish it in time and internal timer in workers seemed more
...
{
    uchar val = 1;//variable to receive values from other components
    int serving = 1;//keep it alive
    while(serving){
        select{
...
void button_listener(chanend c_buttons[3])
{
    uchar val = 1;
    int buttonSW1 = 0, r;
    while(buttonSW1 == 0) { //waits button SW1 to be pressed
...
void worker(chanend c_distributor, chanend nextWorker, chanend prevWorker, int id)
{
    ...
    int serving = 1;//keep worker alive
    int overflows = 0;//used to count overflows in timer
    int count_ovflws = 1;//used to detect overflows in timer
    ...
    while(serving){
        counter++;
        timer t_wrk_com;//timer to time communication betwen workers
        uint32_t start_wrk_com, end_wrk_com;//to count time elapsed for worker communication
        t_wrk_com :> start_wrk_com;//start counter right before communication
        ...
        t_wrk_com :> end_wrk_com;//end timer after worker commuication
        //print measured time of communication (was used for time measurements, it is commented because it is not the purpose of the program)
        //printf("id %d time elapsed for communication between workers is: %d\n",id,(end_wrk_com-start_wrk_com)/1000000);

        timer t_comp;//timer used to time a computation cycle
        uint32_t start_comp, end_comp;//to store start and end of computation cycle
        t_comp :> start_comp;//start computation timing
        ...
        t_comp :> end_comp;//stop timer after computations
        //print computation times (was used for time measurements, it is commented because it is not the purpose of the program)
        //printf("id %d time elapsed for computations: %d\n", id, (end_comp-start_comp)/1000000);

        //detect overflows for the "big timer" which counts a whole cycle including computation and communication
        uint32_t temp = 0;//used to detect an overflow
        t_work :> temp;
        if(temp <= start_work && count_ovflws){//if the timer has restarted, we count an overflow
            overflows++;
            t_work :> start_work;
            count_ovflws = 0;//we do not count overflows twice in one cycle
        }
        if(temp > start_work)
            count_ovflws = 1;//we turn this off to count overflows in coming cycles
        //write time when 100 cycles are executed
        if(counter == 100){
            t_work :> end_work;
            //print total cycle time (was used for time measurements, it is commented because it is not the purpose of the program)
            //printf("time elapsed for 100 cycles in worker thread: %dms\n", (overflows*4200)+((end_work-start_work)/1000000));
        }
        //select statement to see if worker needs to output image state
        select {
            case c_distributor :> val:
                    //if distributor commands worker to work, we continue as usual
                    if(val == 0)
                        break;
                    //if distributor commands worker to output image, send image state (of the part of the image it has) to distributor
                    if(val == 1) {
                        for( int y = 1; y < strip_size+1; y++) {   //go through all lines
                            for( int x = 0; x < IMWD_; x++ ) { //go through each uchar per line
...
/////////////////////////////////////////////////////////////////////////////////////////
//
// Timer thread triggered by the distributor (not used)
//
/////////////////////////////////////////////////////////////////////////////////////////
void timer_thread(chanend c_dist)
{
    uint32_t start, end = 0;//store start, end of time
    uchar activate = 0;//receive activation/deactivation signals
    timer t;
    while(1){
        select{
...
chan c_inIO, c_control, c_outIO;    //data input stream - distributor, orientation - distributor, distributor - data output stream
chan c_out_workers [nb_workers];    //distributor - workers
chan chan_workers [nb_workers];     //workers - workers
chan c_buttons[3];                  //button listener - distributor, button listener - data input stream, button listener - data output stream
chan c_leds[3];                     //leds handler - distributor, leds handler - data input stream, leds handler - data output stream
...
par {
    ...
    //worker threads
    on tile[1]: worker(c_out_workers[0], chan_workers[0], chan_workers[7],0);
    on tile[1]: worker(c_out_workers[1], chan_workers[1], chan_workers[0],1);
    on tile[1]: worker(c_out_workers[2], chan_workers[2], chan_workers[1],2);
    on tile[1]: worker(c_out_workers[3], chan_workers[3], chan_workers[2],3);
